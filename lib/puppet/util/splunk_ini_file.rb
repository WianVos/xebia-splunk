...
require File.expand_path('../splunk_ini_file/section', __FILE__)

module Puppet
  module Util
    class Splunk_IniFile
      ...
      def has_section?(section_name)
        result = false
        result = true if @sections_hash.has_key?(section_name)
      end

      def remove_section(section_name)
        @sections_hash.delete(section_name) if  @sections_hash.has_key?(section_name)
        @section_names.delete(section_name)  if @section_names.include?(section_name)
      end
      ...
      def get_settings(section_name)
        if  @sections_hash[section_name] != nil
      ...
      def lines
        @lines ||= Splunk_IniFile.readlines(@path)
      end
